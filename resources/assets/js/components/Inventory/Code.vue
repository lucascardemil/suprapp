<template>
    <div>
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" id="pills-product-exist-tab" data-toggle="pill" href="#pills-product-exist"
                    role="tab" aria-controls="pills-product-exist" aria-selected="true">Producto Existente</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="pills-new-product-tab" data-toggle="pill" href="#pills-new-product" role="tab"
                    aria-controls="pills-new-product" aria-selected="false">Nuevo Producto</a>
            </li>
            <li class="nav-item">
        ...
        <div class="tab-content bg-white p-3" id="pills-tabContent">
            <div class="tab-pane fade show active" id="pills-product-exist" role="tabpanel"
                aria-labelledby="pills-product-exist-tab">
                <form action="POST" v-on:submit.prevent="createCode">
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-6">
                            ...
                            <input required type="text" name="codigo" class="form-control" v-model="newCode.codebar">
                        </div>
                        <div class="form-group col-lg-6">
                            <label for="atributo">Atributo</label>
                            <input required type="number" name="atributo" class="form-control" v-model="newCode.atributo">
                        </div>
                    </div>
            ...
            <table class="table table-hover table-striped mt-3 table-sm text-white bg-dark">
                ...
                <tbody>
                    <tr v-for="codeLocal in rows" :key="codeLocal.id">
                        ...
                        <td v-else></td>
                        <td v-if="codeLocal.productpagos != null">${{ codeLocal.productpagos.flete }}</td>
                        <td v-else></td>
                        <td width="150px">
                            <a href="#" class="btn btn-warning btn-sm" @click.prevent="editCode({ codeLocal })"
                                data-toggle="tooltip" data-placement="top" title="Editar">
                                <i class="far fa-edit"></i>
                            </a>
                            <a href="#" class="btn btn-danger btn-sm"
                                @click.prevent="modalDeleteCode({ id: codeLocal.product.id })" data-toggle="tooltip"
                                data-placement="top" title="Eliminar">
                                <i class="fas fa-ban"></i>
                            </a>
                            <a href="#" class="btn btn-info btn-sm" @click.prevent="modalInventory({ id: codeLocal.id })"
                                data-toggle="tooltip" data-placement="top" title="Inventariado">
            ...
            <ul class="pagination">
                <li class="page-item" v-if="pagination.current_page > 1">
                    <a class="page-link border-light bg-dark" href="#"
                        @click.prevent="changePageCode({ page: 1 })"><span>Primera</span></a>
                </li>
                <li class="page-item" v-if="pagination.current_page > 1">
                    <a class="page-link border-light bg-dark" href="#"
                        @click.prevent="changePageCode({ page: pagination.current_page - 1 })"><span>Atrás</span></a>
                </li>
                <li class="page-item" v-for="page in pagesNumber" v-bind:class="[page == isActived ? 'active' : '']"
                    :key="page">
                    <a class="page-link border-light bg-dark" href="#" @click.prevent="changePageCode({ page })">{{ page
                    }}</a>
                </li>
                <li class="page-item" v-if="pagination.current_page < pagination.last_page">
                    <a class="page-link border-light bg-dark" href="#"
                        @click.prevent="changePageCode({ page: pagination.current_page + 1 })"><span>Siguiente</span></a>
                </li>
                <li class="page-item" v-if="pagination.current_page < pagination.last_page">
                    <a class="page-link border-light bg-dark" href="#"
                        @click.prevent="changePageCode({ page: pagination.last_page })"><span>Última</span></a>
                </li>
            </ul>
        </nav>
